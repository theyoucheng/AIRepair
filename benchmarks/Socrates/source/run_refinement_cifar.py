    ...
    for i in range(args.num_tests):
        ...
        if lbl_x0 == y0s[i]:
            ...
            while True:
                ...
                if res:
                    print('Verified at {:.4f}'.format(eps))
                    best_verified = max(best_verified, eps)
                else:
                    print('Failed at {:.4f}'.format(eps))
                ...
                if best_verified == round(best_failed - 0.0001, 4): break

                if res == 1:
                    if step_eps == 0.001:
                        eps = round(eps + step_eps, 4)
                    elif step_eps == -0.0005:
                        step_eps = 0.0001
                        eps = round(eps + step_eps, 4)
                    elif step_eps == 0.0001:
                        eps = round(eps + step_eps, 4)
                elif res == 0:
                    if step_eps == 0.001:
                        step_eps = -0.0005
            ...
            print("Image {} Verified at {:.4f} and Failed at {:.4f}".format(i, best_verified, best_failed))
        else:
            print('Skip at data {}'.format(i))
            print("Image {} Verified at {:.4f} and Failed at {:.4f}".format(i, -1, -1))
            res = -1
