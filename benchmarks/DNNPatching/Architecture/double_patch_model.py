...
class Model(nn.Module):
    def __init__(self):
        ...
        self.datch_output_channels_counts = [4, 4, 8, 8, 16, 32]
        self.datch_channels_count = [3, 4+4+8, 4+4+8, 8+8+16, 8+8+16, 16+16+32, 32+32+64]
        #exp02
        ...
        self.datch_fc_features_output_counts = [32, self.labels_count]
        ...
        self.datch_fc_features = [self.status_inputs_count + self.datch_channels_count[-1]*2*2,
                                  self.fc_features[1] + self.patch_fc_features_output_counts[0] + self.datch_fc_features_output_counts[0]]
        ...
        if args.task == 'oa':
            ...
            self.datch_fc_aux_features_output_counts = [32, self.aux_labels_count]
            self.datch_fc_aux_features = [self.datch_channels_count[-1]*2*2,
                                          self.datch_fc_features_output_counts[0]]
            ...
            if args.train_patch:
                ...
                self.initialize_datch_layers(
                    self.datch_channels_count, self.datch_output_channels_counts,
                    self.datch_fc_features, self.datch_fc_features_output_counts,
                    self.datch_fc_aux_features, self.datch_fc_aux_features_output_counts
                )
        else:
            self.initialize_training_layers(self.channel_counts, self.fc_features)
            if args.train_patch:
    ...
    def initialize_datch_layers(self, channel_counts_in, channel_counts_out,
                                 fc_features_in, fc_features_out,
                                 fc_aux_features_in=None, fc_aux_features_out=None):

        self.datch_rd_activation = nn.Sequential(nn.ReLU(True), nn.Dropout(self.datch_dropout_rate))

        if self.datch_output_channels_counts[0] != 0:
            self.a1_datch, self.b1_datch = self.resnet_subblocks(
                in_channels=channel_counts_in[0], out_channels=channel_counts_out[0], kernel_size=3)
        if self.datch_output_channels_counts[1] != 0:
            self.a2_datch, self.b2_datch = self.resnet_subblocks(
                in_channels=channel_counts_in[1], out_channels=channel_counts_out[1], kernel_size=3)
        if self.datch_output_channels_counts[2] != 0:
            self.a3_datch, self.b3_datch = self.resnet_subblocks(
                in_channels=channel_counts_in[2], out_channels=channel_counts_out[2], kernel_size=3)
        if self.datch_output_channels_counts[3] != 0:
            self.a4_datch, self.b4_datch = self.resnet_subblocks(
                in_channels=channel_counts_in[3], out_channels=channel_counts_out[3], kernel_size=3)
        if self.datch_output_channels_counts[4] != 0:
            self.a5_datch, self.b5_datch = self.resnet_subblocks(
                in_channels=channel_counts_in[4], out_channels=channel_counts_out[4], kernel_size=3)


        if self.datch_output_channels_counts[5] != 0:
            self.fe_datch = nn.Sequential(
                nn.Conv2d(channel_counts_in[5], channel_counts_out[5], 3, 1, padding=0),
                nn.BatchNorm2d(channel_counts_out[5]),
                self.datch_rd_activation)

        datch_fc_layers = []
        for i in range(len(fc_features_in)):
            lin_layer = nn.Linear(fc_features_in[i], fc_features_out[i])
            datch_fc_layers += [lin_layer, self.str_rd_activation]
        self.fc_datch = nn.Sequential(*datch_fc_layers[:-1])

        if args.task == 'oa':
            datch_fc_aux_layers = []
            for i in range(len(fc_aux_features_in)):
                lin_layer = nn.Linear(fc_aux_features_in[i], fc_aux_features_out[i])
                datch_fc_aux_layers += [lin_layer, self.datch_rd_activation]
            self.fc_aux_datch = nn.Sequential(*datch_fc_aux_layers[:-1])
    ...
    def forward(self, inputs):
        ...
        if self.datch_output_channels_counts[0] != 0 and args.train_patch:
            x_datch_a = self.a1_datch(x)
            x_datch_b = self.b1_datch(x)
            x_datch_sum = x_datch_a + x_datch_b
            x_datch_sum = self.datch_rd_activation(x_datch_sum)
        ...
        if self.datch_output_channels_counts[1] != 0 and args.train_patch:
            if self.datch_output_channels_counts[0] != 0:
                x_datch = x_datch_sum
                x_concat = torch.cat((x, x_patch, x_datch), dim=1)
            else:
                x_concat = x
            x_datch_a = self.a2_datch(x_concat)
            x_datch_b = self.b2_datch(x_concat)
            x_datch_sum = x_datch_a + x_datch_b
            x_datch_sum = self.datch_rd_activation(x_datch_sum)
        ...
        if self.datch_output_channels_counts[2] != 0 and args.train_patch:
            if self.datch_output_channels_counts[1] != 0:
                x_datch = x_datch_sum
                x_concat = torch.cat((x, x_patch, x_datch), dim=1)
            else:
                x_concat = x
            x_datch_a = self.a3_datch(x_concat)
            x_datch_b = self.b3_datch(x_concat)
            x_datch_sum = x_datch_a + x_datch_b
            x_datch_sum = self.datch_rd_activation(x_datch_sum)
        ...
        if self.datch_output_channels_counts[3] != 0 and args.train_patch:
            if self.datch_output_channels_counts[2] != 0:
                x_datch = x_datch_sum
                x_concat = torch.cat((x, x_patch, x_datch), dim=1)
            else:
                x_concat = x
            x_datch_a = self.a4_datch(x_concat)
            x_datch_b = self.b4_datch(x_concat)
            x_datch_sum = x_datch_a + x_datch_b
            x_datch_sum = self.datch_rd_activation(x_datch_sum)
        ...
        if self.datch_output_channels_counts[4] != 0 and args.train_patch:
            if self.datch_output_channels_counts[3] != 0:
                x_datch = x_datch_sum
                x_concat = torch.cat((x, x_patch, x_datch), dim=1)
            else:
                x_concat = x
            x_datch_a = self.a5_datch(x_concat)
            x_datch_b = self.b5_datch(x_concat)
            x_datch_sum = x_datch_a + x_datch_b
            x_datch_sum = self.datch_rd_activation(x_datch_sum)
        ...
        if self.datch_output_channels_counts[5] != 0 and args.train_patch:
            if self.datch_output_channels_counts[4] != 0:
                x_datch = x_datch_sum
                x_concat = torch.cat((x, x_patch, x_datch), dim=1)
            else:
                x_concat = x
            x_datch_fe = self.fe_datch(x_concat)
        ...
        if args.train_patch and self.datch_output_channels_counts[5] != 0:
            x_datch = x_datch_fe
            x_datch = x_datch.reshape(-1, self.datch_output_channels_counts[-1]*2*2)
        ...
        if args.task == 'oa':
            if args.train_base:
                ...
                if args.train_patch:
                    ...
                    fc_aux_datch_layers_list = list(self.fc_aux_datch.modules())[1:]
                ...
                fc_aux_datch_layers_list = list(self.fc_aux_datch.modules())[1:]
                if self.datch_fc_aux_features_output_counts[0] != 0:
                    if self.datch_output_channels_counts[5] != 0:
                        x_concat = torch.cat((x, x_patch, x_datch), dim=1)
                    else:
                        x_concat = x
                    x_datch_fc_aux = fc_aux_datch_layers_list[0](x_concat)
                    x_datch_fc_aux = fc_aux_datch_layers_list[1](x_datch_fc_aux)

                if self.datch_fc_aux_features_output_counts[1] != 0:
                    if self.datch_fc_aux_features_output_counts[0] != 0:
                        x_concat = x_datch_fc_aux #torch.cat((x_patch_fc_aux), dim=1)
                    else:
                        #TODO handle error properly
                        print("Error")
                        sys.stop()
                    x_datch_fc_aux = fc_aux_datch_layers_list[4](x_concat)
        ...
        if args.train_patch:
            ...
            fc_datch_layers_list = list(self.fc_datch.modules())[1:]
        ...
        if args.train_patch and self.datch_fc_features_output_counts[0] != 0:
            if self.datch_output_channels_counts[5] != 0:
                x_concat = torch.cat((x, x_patch, x_datch), dim=1)
            else:
                x_concat = x
            x_datch_fc = fc_datch_layers_list[0](x_concat)
            x_datch_fc = fc_datch_layers_list[1](x_datch_fc)
        ...
        if args.train_patch and self.datch_fc_features_output_counts[1] != 0:
            if self.datch_fc_features_output_counts[0] != 0:
                x_datch = x_datch_fc
                x_concat = torch.cat((x, x_patch, x_datch), dim=1)
            else:
                x_concat = x
            x_datch_fc = fc_datch_layers_list[4](x_concat)
       
        if args.task == 'oa':
            if args.train_patch:
                out = torch.cat((x_fc, x_patch_fc, x_datch_fc_aux, x_datch_fc), dim=1)
