...
grads_baseline_natural = pickle.load(open("grads_baseline_natural.pkl", "rb")) # grads.pkl natural loss
grads_ourbatchnorm_natural = pickle.load(open("grads_ourbatchnorm_natural.pkl", "rb")) # grads.pkl --replace natural loss


# In[ ]:


lol_batch_0 = np.array(grads_baseline_natural).flatten()
lol_batch_1 = np.array(grads_ourbatchnorm_natural).flatten()
...
plt.fill_between(range(len(min_batch_curve)//5+1), min_batch_curve[::5], max_batch_curve[::5], label='natural loss + baseline')
plt.fill_between(range(len(min_batch_curve_1)//5+1), min_batch_curve_1[::5], max_batch_curve_1[::5], label='natural loss + our BatchNorm')
...
plt.fill_between(steps, min_batch_curve[::step], max_batch_curve[::step],
                alpha=0.5, color='C1', label='natural loss + baseline')
...
plt.plot(steps, max_batch_curve[::step], color='C1', label='natural loss + baseline')
plt.plot(steps, max_batch_curve_1[::step], color='C2', label='natural loss + our BatchNorm')
