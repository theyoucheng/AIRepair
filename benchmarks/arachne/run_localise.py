...
def compute_gradient_to_output(path_to_keras_model, 
	...
	if not on_weight:	
		...
		for chunk in chunks:
			_gradient = tf.compat.v1.keras.backend.get_session().run(tensor_grad, feed_dict={model.input: X[chunk]})[0]
		...
		gradients = []
		for chunk in chunks:
			_gradients = tf.compat.v1.keras.backend.get_session().run(tensor_grad, feed_dict={model.input: X[chunk]})
			if len(gradients) == 0:
...
def compute_gradient_to_loss(path_to_keras_model, idx_to_target_layer, X, y, 
	...
	for chunk in chunks:
		_gradients = tf.compat.v1.keras.backend.get_session().run(
