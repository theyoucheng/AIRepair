...
fid_filedir="final_data/data/rq1_fault_ids/"
logdir="logs/rq1"
echo $pwd
if [ ! -d "$logdir" ]; then
	mkdir -m777 -p $logdir
	mkdir -m777 -p $logdir/fm
	mkdir -m777 -p $logdir/c10
	mkdir -m777 -p $logdir/gtsrb
fi
...
./run_mdl.sh 2 final_data/data cifar10 results/rq2/cm/localiser/model.rq2.0.pkl 0 cnn1 